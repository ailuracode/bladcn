...
@php
    ...
    $classes = [$class];
    $attrs = ['id' => $id, 'style' => $style, 'data-slot' => 'combobox'];
@endphp
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('combobox', (open, multiple, defaultValue) => ({
                ...
                $items: null,
                init() {
                    this.$watch('open', value => this.$dispatch(
                        'open-change', value));
                    this.$watch('selected', value => this
                        .$dispatch('selected', value));
                    this.$content = this.$refs.combobox
                        .querySelector(
                            '[data-slot="combobox-content"]');
                    this.$items = Array.from(this.$refs.combobox
                            .querySelectorAll(
                ...
                selectItem(item) {
                    if (this.isMultiple) {
                        ...
                        if (this.selected.includes(item)) this
                ...
                isSelected(text) {
                    return this.isMultiple ? this.selected
                        .includes(text) : this.selected ===
                        text;
                },
                removeItem(item) {
                    if (this.isMultiple) this.selected = this
                        .selected.filter(i => i !== item);
                    else if (this.selected === item) this
                        .selected = null;
                },
            }));
        });
