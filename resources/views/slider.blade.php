...
@php
    ...
    if ($orientation->isVertical()) {
        ...
        if (count($values) === 1) {
            $rangeStyle =
                'height:' . $percentages[0] . '%; bottom:0; width:100%;';
        } else {
            $sorted = $values;
            sort($sorted);
            ...
            $width = $end - $start;
            $rangeStyle =
                'bottom:' . $start . '%; height:' . $width . '%; width:100%;';
        }
    } else {
        $thumbStyles = array_map(function ($p) {
        ...
        if (count($values) === 1) {
            $rangeStyle =
                'width:' . $percentages[0] . '%; left:0; height:100%;';
        } else {
            $sorted = $values;
            sort($sorted);
            $start = $percent($sorted[0]);
            ...
            $width = $end - $start;
            $rangeStyle =
                'left:' . $start . '%; width:' . $width . '%; height:100%;';
        }
    }

    $base =
        ($orientation->isVertical() ? 'flex-col h-40 w-auto' : 'w-full') .
...
<div {{ $isDisabled ? 'aria-disabled=true' : '' }}
    {{ $attributes->class($classes)->merge($attrs) }}
    x-data="slider({ min: {{ $min }}, max: {{ $max }}, step: {{ $step }}, orientation: '{{ $orientation->value }}', values: @js($values), decimals: {{ $decimals }}, disabled: {{ $isDisabled ? 'true' : 'false' }} })">
    <div class="bg-muted {{ $orientation->isVertical() ? 'w-1 h-full' : 'h-1 w-full' }} relative grow overflow-hidden rounded-full"
            ...
            Alpine.data('slider', ({
                ...
                jumpTo(e) {
                    ...
                    const rect = this.track
                        .getBoundingClientRect();
                    let percent = this.orientation ===
                        'vertical' ? (rect.bottom - e.clientY) /
                        rect.height : (e.clientX - rect.left) /
                        rect.width;
